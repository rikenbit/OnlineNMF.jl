...
function each_sparse_dnmf(
    ...
    while (stop == 0 && s <= numepoch)
        ...
        checkNaN(U)
        # Update V
        V = update_spdV(input, N, M, U, V, L, beta, binv, terv, graphv, l1v, l2v, chunksize)
        # NaN
        checkNaN(V)
        # save log file
...
function update_spdU(input, N, M, U, V, beta, binu, teru, l1u, l2u, chunksize)
    numer = update_spdU_numer_BETA(input, N, M, U, V, beta, binu, teru, chunksize)
    denom = update_dU_denom_BETA(N, U, V, beta, binu, teru, l1u, l2u, chunksize)
    update_factor = ifelse.(denom .== 0, 1.0, (numer ./ denom) .^ rho(beta))
    U .* update_factor
end

function update_spdU_numer_BETA(input, N, M, U, V, beta, binu, teru, chunksize)
    tmpN = zeros(UInt32, 1)
    tmpM = zeros(UInt32, 1)
    numer = zeros(size(U))
...
function update_spdV(input, N, M, U, V, L, beta, binv, terv, graphv, l1v, l2v, chunksize)
    numer = update_spdV_numer_BETA(input, N, M, U, V, beta, binv, terv, chunksize)
    denom = update_dV_denom_BETA(N, U, V, L, beta, binv, terv, graphv, l1v, l2v, chunksize)
    update_factor = ifelse.(denom .== 0, 1.0, (numer ./ denom) .^ rho(beta))
    V .* update_factor
end

function update_spdV_numer_BETA(input, N, M, U, V, beta, binv, terv, chunksize)
    numer = zeros(size(V))
    open(input, "r") do file
        stream = ZstdDecompressorStream(file)
        tmpN = zeros(UInt32, 1)
