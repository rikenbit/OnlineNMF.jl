...
function parse_commandline(nmfmodel::Union{NMF,SPARSE_NMF,BINCOO_NMF})
    ...
    @add_arg_table! s begin
        ("--input", "-i")
        help = "Julia Binary file generated by `OnlinePCA.csv2bin` or `OnlinePCA.mm2bin` function."
        ...
        default = 1
        ("--algorithm", "-g")
        help = "The algorithm of NMF. Choose from `frobenius`, `kl`, `is`, `pearson`, `hellinger`, `neyman`, `alpha`, and `beta`."
        arg_type = AbstractString
        default = "frobenius"
        "--lower"
