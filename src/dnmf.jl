...
function dnmf(;input::AbstractString="",
    ...
    nmfmodel = DNMF()
    beta, binu, binv, teru, terv, graphv, l1u, l1v, l2u, l2v, U, V, L, N, M, dim, numepoch, chunksize, lower, upper = init_dnmf(input, outdir, beta, binu, binv, teru, terv, graphv, l1u, l1v, l2u, l2v, dim, numepoch, chunksize, initU, initV, initL, logdir, lower, upper, nmfmodel)
    # Perform DNMF
    out = each_dnmf(input, beta, binu, binv, teru, terv, graphv, l1u, l1v, l2u, l2v, U, V, L, N, M, dim, numepoch, chunksize, lower, upper, nmfmodel)
...
function each_dnmf(input, beta, binu, binv, teru, terv, graphv, l1u, l1v, l2u, l2v, U, V, L, N, M, dim, numepoch, chunksize, lower, upper, nmfmodel)
...
function update_dU(input, N, M, U, V, beta, binu, teru, l1u, l2u, chunksize)
    ...
    denom = update_dU_denom_BETA(U, V, beta, binu, teru, l1u, l2u, chunksize)
...
function update_dU_denom_BETA(U, V, beta, binu, teru, l1u, l2u, chunksize)
    numer = zeros(size(U))
    n = 1
    while n <= N
        batch_size = min(chunksize, N - n + 1)
        U_chunk = @view U[n:n+batch_size-1, :]
        @turbo numer[n:n+batch_size-1, :] = ((U_chunk * V').^(beta - 1)) * V .+ l1u + l2u .* U_chunk + binu .* (2 .* U_chunk.^3 .+ U_chunk) + teru .* (6 .* U_chunk.^5 .+ 52 .* U_chunk.^3 .+ 8 .* U_chunk)
        n += batch_size
    end
    return numer
end
...
function update_dV(input, N, M, U, V, L, beta, binv, terv, graphv, l1v, l2v, chunksize)
    ...
    denom = update_dV_denom_BETA(U, V, L, beta, binv, terv, graphv, l1v, l2v, chunksize)
    update_factor = ifelse.(denom .== 0, 1.0, (numer ./ denom) .^ rho(beta))
    V .* update_factor
end
...
function update_dV_denom_BETA(U, V, L, beta, binv, terv, graphv, l1v, l2v, chunksize)
    numer = zeros(size(V))
    n = 1
    while n <= N
        batch_size = min(chunksize, N - n + 1)
        U_chunk = @view U[n:n+batch_size-1, :]
        @turbo numer .+= ((U_chunk * V') .^ (beta - 1))' * U_chunk .+ l1v + l2v .* V + binv .* (2 .* V.^3 .+ V) + terv .* (6 .* V.^5 .+ 52 .* V.^3 .+ 8 .* V) + graphv .* (L * V)
        n += batch_size
    end
    return numer
end
...
function init_dnmf(input::AbstractString, outdir::Union{Nothing,AbstractString},
    ...
    return beta, binu, binv, teru, terv, graphv, l1u, l1v, l2u, l2v, U, V, L, N, M, dim, numepoch, chunksize, lower, upper
end
